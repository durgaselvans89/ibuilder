...
var keyPad = function() {
	 ...
	 *  @return this
	*///=====================================================================================================
	function init(p_keypadDiv, p_closeFunction,p_playClickFunction, p_playNegsfxFunction, p_hasFractionFlag, p_charLimit,p_keypadNumber) {
		...
		if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
            ...
            _$thisKeypad.find(".closebutton").on("mousedown", userClickedCloseButton);
            _$thisKeypad.find(".custom_symbol_key, .number_symbol_key, .enter_key, .space_key, .shift_key, .alphabet_key, .keys_to_show").on("mousedown", keyClickHandler);
	...
	function removeTheHilite(e){
		if(_clickedKeyClassName){
			  _$clickedKey.removeClass("downstate");
			 _clickedKeyClassName = null;
			 _$clickedKey 	= null;
		   }	
	...
	*///=====================================================================================================
	function userClickedAlphabetNumberOrSpaceKey(p_Event,p_KeycodeAsString){
		...
		if(_currEntryMode === _MODES.DISPLAY){
		   ...
		   _$clickedKey.addClass("downstate");
	...
	*///=====================================================================================================
	function keyClickHandler(e) {
		...
		if(_$clickedKey.hasClass("number_symbol_key")){
			...
			if((_currEntryMode === _MODES.MAKEFRACTION )&&(["162","36","8457","8451"].indexOf(tString) > -1)){ return };
	...
	function setBlinkingCursorDistance(){
		...
		tMoveTo += "px"
		_$blinkingCursor.css({"visibility":"visible","left":tMoveTo});
	}
	...
	*///=====================================================================================================
	function userClickedKeysToShowButton(e){
		...
		if(_keysShowing === _MODES.keysABC){
			...
			_$thisKeypad.find(".shift_key").css("opacity","0.5")
			_$thisKeypad.find(".keys_to_show").css("background-position","0px -49px");
			...
			_$thisKeypad.find(".shift_key").css("opacity","1")
			_$thisKeypad.find(".keys_to_show").css("background-position","0px 0px");
			_$thisKeypad.find(".groupABC").css("z-index",1);
			_$thisKeypad.find(".group123").css("z-index",-1);		
		}
	}
    ...
    function userClickedShiftKey(event){
	   ...
	   playClickSFX();
		...
		if(	_letterCaseMode === _MODES.LOWERCASE){
			...
			_$thisKeypad.find(".shift_key").css("background-position","0px -48px");
            for(var i = 0; i < 26; i++){
	            var tNum = i + 65;
			...
			_$thisKeypad.find(".shift_key").css("background-position","0px 0px");	
	        for(var i = 0; i < 26; i++){
		         var tNum = i + 97;
				 _$thisKeypad.find(".letter_" + tArray[i]).html("&#"+tNum +";");
	        }
	...
	function userClickedOnEnterKey(event) {	
	    ...
	    _$thisKeypad.find(".enter").css({"visibility":"hidden","background-position":"0px 0px"});
	...
	*///=====================================================================================================
	function userClickedMakeFractionKey(e) {
		...
		_$thisKeypad.find(".enter").css("visibility","visible");
		...
		for (var i = 0; i < tMasterLen; i++){
			...
			tString = _masterHtmlArray[i];
				...
				if (tString.indexOf(FRACTION_DELIMITER) !== -1){
					...
					// ADD string that represents fraction
					// CHANGED to accomodate using EM4 font
					//$\frac{\$100.00}{\$10.00}$
					//var tNewString  = "&#92" + "(" + "&#92" + "frac{" + tTop + "}{" + tBot + "}" + "&#92" + ")";
					
					var tNewString  = "&#92;" + "(" + "&#92;" + "frac{"+ "&#92;" +"style{font-family:reportschool, TeX, Arial, sans-serif;}{"+ "&#92;" +"text{" + tTop +"}}}{"+"&#92;"+ "style{font-family:reportschool, Tex, Arial, sans-serif;}{"+ "&#92;" +"text{"+ tBot + "}}}"+ "&#92;" + ")"
					//var tNewString  = "&#92" + "(" + "&#92" + "frac{"+ "&#92" +"style{font-family:reportschool, TeX, Arial, sans-serif;}{"+ "&#92" +"text{" + tTop +"}}}{"+"&#92"+ "style{font-family:reportschool, Tex, Arial, sans-serif;}{"+ "&#92" +"text{"+ tBot + "}}}"+ "&#92" + ")"
					//OLD WAY - USES EM FONT --> var tNewString  = "&#92" + "(" + "&#92" + "frac{" + ""+ tTop2 + "}{" +""+ tBot2 + "}" + "&#92" + ")";
					//console.log(tNewString);
				     //var tNewString  = "&#92" + "(" + "&#92" + "frac{" + tTop + "}{" + tBot + "}" + "&#92" + ")";
				     var tNewString  =  "\\(\\frac{\\style{font-family:reportschool, Arial, sans-serif;}{\\text{" + tTop +"}}}{\\style{font-family:reportschool, Tex, Arial, sans-serif;}{\\text{"+ tBot + "}}}\\)"
				     //var tNewString  = "\\(\\frac{\\style{font-family:Arial, sans-serif}{\\text{"+tTop+"}}}{\\style{font-family:Arial,sans-serif}{\\text{"+tBot+‌​"}}}\\)";
						
					_stringOfHtml +=  tNewString;					
					
				} else {
					// wrap dollar sign $ in block to nuetralize mathjax using it as delimiter
					if (tString === "36"){
						// dollar sign
						_stringOfHtml += ("<block>&#36;</block>" );
					}else if(tString === "8457"){
						// fahrenheit and celsius are rendered slightly smaller
						_stringOfHtml += ("<block style='font-size:95%;'>&#8457;</block>");
					}else if (tString === "8451"){
						_stringOfHtml += ("<block style='font-size:95%;'>&#8451;</block>");
					}else{
						// regular character
			...
			if(_fractionMode === _MODES.TOPFRACTION){
					...
					for (var i = 0; i < tLen; i++){
						...
						// wrap $ in block to nuetralize mathjax using it as delimiter
						if (tString === "36"){
							_stringOfHtml += ("&#" + 36 + ";");	
						}else if(tString === "8457"){
							_stringOfHtml += ("<block style='font-size:12px;'>&#8457;</block>");
						}else if (tString === "8451"){
							_stringOfHtml += ("<block style='font-size:12px;'>&#8451;</block>");
						}else
							_stringOfHtml += ("&#" + tString + ";");	
