...
var page_builder_app = {
	...
	pageBuilder: function ( event ) {
		console.log("ps210 pageBuild.pageBuilder --- ");
	...
	contentArr += '<div id="content" data-template="'+template+'" data-grades="'+$.json_grades+'" data-layout="'+$.json_layout+'" data-id="0" data-font="'+set_font_size+'"><div class="content">'; //data-iteration="'+iteration+'"
		...
		if (typeof JSONObject.coordinate_object !== 'undefined') {
			...
			$.each(JSONObject.coordinate_object, function(index) {
				// variables
				$.coordOb_dimension_x = 400;
				$.coordOb_dimension_y = 400;
				$.coordOb_scale_x = 15;
				$.coordOb_scale_y = 15;
				...
				// get block size
				$.coordOb_blocksize_x = Math.round($.coordOb_dimension_x/$.coordOb_scale_x);
				$.coordOb_blocksize_y = Math.round($.coordOb_dimension_y/$.coordOb_scale_y);

				// get dimensions
				$.coordOb_dimension_x = $.coordOb_blocksize_x * $.coordOb_scale_x;
				$.coordOb_dimension_y = $.coordOb_blocksize_y * $.coordOb_scale_y;
				...
				contentArr += '<div id="coordinateGrid" style="width:'+$.coordOb_dimension_x+'px;height:'+$.coordOb_dimension_y+'px;">';

				// add in quadrant and blocks
				contentArr += '<div class="q1" style="overflow:hidden;float:left;width:'+$.coordOb_dimension_x+'px;border: 1px solid black;">';
				for (var i = 0; i < $.coordOb_scale_y; i++) {
					var style = 'width:'+$.coordOb_blocksize_x+'px;height:'+$.coordOb_blocksize_y+'px;float:left;z-index: 1;box-sizing: border-box;-moz-box-sizing: border-box;-webkit-box-sizing: border-box;border-right: 1px solid rgba(0,0,0,0.3);border-bottom: 1px solid rgba(0,0,0,0.3);';
				...
				if (JSONObject.coordinate_object[index].scale !== "") {
					contentArr += '<div class="scale" style="position:absolute;left:10px;top:10px;">'+JSONObject.coordinate_object[index].scale+'</div>';
				}
				...
				contentArr += '<div id="controls" style="position:absolute;left:0;top:-50px;"><button id="resetButton" style="'+buttonStyles+'">Reset</button></div>';
...
function withjQuery (isTemplate) {
    ps210.init(JSONObject, isTemplate);
}
function polljQuery (time, isTemplate) {
  if (window.jQuery && $(".content_boxs").length > 0 && window.ps210) {
