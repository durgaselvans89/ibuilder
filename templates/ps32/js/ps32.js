var ps32 = 
  {
    ...
    init: function () {		       
		...
		if(typeof HTML5RiaAPI !== "undefined") {
            ...
            cogAPI.setCaptureImageFromAPPFunction(ps32.getCanvas);
        }
        ...
        for (i = 0; i < $drags.length; i++) {
            ...
            $drags.eq(i).draggable({
                revert: function(event,ui) {
                    ...
                    $(this).data("uiDraggable").originalPosition = {top:ps32.dragLookup[index].orig.top,left:ps32.dragLookup[index].orig.left};
                    ps32.dragLookup[index].wasInBox = !!ps32.dragLookup[index].inBox;
                    if(!event) {
                        ps32.dragLookup[index].inBox = false;
                        ps32.dragLookup[index].curr.left = ps32.dragLookup[index].orig.left;
                        ps32.dragLookup[index].curr.top = ps32.dragLookup[index].orig.top;
                    }
                    return !event;
                },
                stop: function(event,ui) {
                    var index = $(this).attr("c_id") || $(this).attr("data-id");
                    if (ps32.dragLookup[index].wasInBox || ps32.dragLookup[index].inBox)
                        ps32.captureScreenShot();
                },
                cursor: "pointer"
            });
        ...
        for (i = 0; i < $drops.length; i++) {
            $drops.eq(i).droppable({
                ...
                "drop": function(event,ui) {
                    ...
                    ps32.dragLookup[drag_id].inBox = $(this).attr('data-id');
                    ps32.dragLookup[drag_id].curr = ui.draggable.position();
                },
		...
		$options.each(function( index ) {
			ps32.optionLookup[cnt] = {el:$(this),id:$(this).attr('data-id'),select:"0"};
			cnt++;
		});		
        $('.answer').click(function() {
            ...
            if(JSONObject.single && ps32.lastAns != null && ps32.lastAns.attr("data-id") != $( this ).attr("data-id") && ps32.lastAns.attr("data-select") == "1"){
                ps32.lastAns.attr("data-select","0");
            }
            ps32.lastAns = $( this ); 
            ps32.captureScreenShot();         
        });
        ...
        MathJax.Callback.Queue([ps32.captureScreenShot]);
    },
    ...
    reset: function(state) {
        if (state != undefined) {
            ...
            for (var a in s.drag) {
                ps32.dragLookup[a].orig.top = s.drag[a].orig.top;
                ps32.dragLookup[a].orig.left = s.drag[a].orig.left;
                ps32.dragLookup[a].el.css({top:s.drag[a].curr.top,left:s.drag[a].curr.left});
                ps32.dragLookup[a].curr.top = s.drag[a].curr.top;
                ps32.dragLookup[a].curr.left = s.drag[a].curr.left;
                if (s.drag[a].inBox != undefined) {
                    ps32.dragLookup[a].inBox = s.drag[a].inBox;
                }
            }			
            for (var a in s.option) {				
                ps32.optionLookup[a].el.attr("data-select",s.option[a].select);
            }
        } else {
            for (var a in ps32.dragLookup) {
                ps32.dragLookup[a].el.css({top:ps32.dragLookup[a].orig.top,left:ps32.dragLookup[a].orig.left});
                ps32.dragLookup[a].curr.top = ps32.dragLookup[a].orig.top;
                ps32.dragLookup[a].curr.left = ps32.dragLookup[a].orig.left;
                ps32.dragLookup[a].inBox = false;
            }
			for (var a in ps32.optionLookup) {
                ps32.optionLookup[a].el.attr("data-select","0");                
            }
        }
        ps32.lastAns = null; 
        ps32.captureScreenShot();
    ...
    getState: function () {
        var drags =  jQuery.extend(true, {},ps32.dragLookup);
		...
		$('.answer').each(function( index ) {
			ps32.optionLookup[cnt] = {el:$(this),id:$(this).attr('data-id'),select:$(this).attr('data-select'),ans:$(this).attr('data-correct')=="true"? 1: 0};			
			inOptions.push(cnt+"/"+$(this).attr('data-select'));
			cnt++;
		});	
		var options =  jQuery.extend(true, {},ps32.optionLookup);
        var ret = {drag: drags, "inDrops": inDrops.join("|"),option:options,"inOptions":inOptions.join("|")};
        return ret;
    },
    getCanvas: function () {
        if (ps32.ssCanvas != null) 
            return ps32.ssCanvas.toDataURL();
...
function getState() {
    var state = jQuery.extend(true, {}, ps32.getState());
    var ret = {};
    var drags = [];
    var options = [];
