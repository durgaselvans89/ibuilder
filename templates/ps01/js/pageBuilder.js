...
var page_builder_app = {
	...
	// END new content wrapper
		...
		$.getScript("js/ps01.js");
	    	$.getScript("Grading/grading.js");
        }
        polljQuery(30);

        var remaining = 422 - $("#title_set").height();
        $("#content_box_set").css("max-height",remaining/2 + "px");
        $(".drop_box").css("max-height",(remaining/2 - 10) + "px");
	...
	regexFun: function ( event ) {
		...
		var index = 0;
		var regex_arr = [];
        //This filters out the text in surrounding the ##xx## tags and wraps it in a notHighlightable span
        var contents = contentArr.match(/\|\|\|\|([\s\S]*?)\|\|\|\|/g);
        for (var j = 0; j<contents.length;j++) {
            var texts = contents[j].match(/\|\|\|\|([\s\S]*)\|\|\|\|/)[1].split("##");
            for (var i = 0,y = 1; i < texts.length; i+=2,y+=2) {
                texts[i] = "<span class='notHighlightable'>"+texts[i]+"</span>";
                if (y < texts.length)
                    texts[y] = "##"+texts[y]+"##";
            }
            contentArr = contentArr.replace(/\|\|\|\|([\s\S]*?)\|\|\|\|/,texts.join(""));
        }
		contentArr = contentArr.replace(regex, function (match, contents) { 
			...
			case 's':
			  ...
			  break;
			case 'h':
			  return '<span class="highlight" data-id="'+$.type_id+'">' +  $.type_text + '</span>';
			  break;
			case 'i':
...
function polljQuery (time) {
  if (window.jQuery && $(".content_boxs").length > 0 && window.ps01) {
