...
var page_builder_app = {
	...
	pageBuilder: function(event) {
		console.log("ps09 pageBuild.pageBuilder --- ");
		...
		if (typeof JSONObject.coordinate_object !== 'undefined') {
			...
			$.each(JSONObject.coordinate_object, function(index) {
				...
				if (eval(JSONObject.coordinate_object[index].show_origin)) {
				...
				// add in canvas
				contentArr += '<div id="coordinateGridCanvas" style="position:absolute;left:1px;top:1px;"></div>';
				...
				var connect = (eval(JSONObject.coordinate_object[index].connect) && JSONObject.coordinate_object[index].coordinate_object_type === "Point") ? '' : 'display:none;';
				var lineType = (eval(JSONObject.coordinate_object[index].lineOption) && JSONObject.coordinate_object[index].coordinate_object_type === "Line") ? '' : 'display:none;';
				var shade = (eval(JSONObject.coordinate_object[index].shadeOption) && JSONObject.coordinate_object[index].coordinate_object_type === "Line") ? '' : 'display:none;';
				var arrow = (eval(JSONObject.coordinate_object[index].arrowOption) && JSONObject.coordinate_object[index].coordinate_object_type === "Segment") ? '' : 'display:none;';
				var remove = (JSONObject.coordinate_object[index].coordinate_object_type === "Segment" || JSONObject.coordinate_object[index].coordinate_object_type === "Line") ? '' : 'display:none;';
				contentArr += '<div id="controls" style="position:absolute;left:0;top:-50px;"><button id="connect" style="'+buttonStyles+' '+connect+'">Connect</button><button id="arrow" style="'+buttonStyles+' '+arrow+'">Arrow</button><button id="lineType" style="'+buttonStyles+' '+lineType+'">Line Type</button><button id="shade" style="'+buttonStyles+' '+shade+'">Shade</button><button id="delete" style="'+buttonStyles+' '+remove+'">Delete</button><button id="resetButton" style="'+buttonStyles+'">Reset</button></div>';
...
function withjQuery (isTemplate) {
    ps09.init(JSONObject, isTemplate);
}
function polljQuery (time, isTemplate) {
  if (window.jQuery && $(".content_boxs").length > 0 && window.ps09) {
