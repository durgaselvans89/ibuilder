...
var page_builder_app = {
	...
	contentArr += '<div id="content" data-template="'+template+'" data-grades="'+$.json_grades+'" data-layout="'+$.json_layout+'" data-id="0" data-font="'+set_font_size+'"><div class="content">'; //data-iteration="'+iteration+'"
		...
		contentArr += '<div id="content_box_set" class="content_boxs" data-id="0">';
			...
			$.each(JSONObject.content_box, function(index) { 
				...
				{
				case 'txt':
				  contentArr += '<div id="content_box_'+$.json_content_box_id+'" style="z-index:5;'+$.position+$.add_json_width+' "class="content_box" data-block="content_box" data-id="'+$.json_content_box_id+'" data-type="'+$.json_content_box_type+'" data-name="'+$.json_content_box_name+'"><span class="txt">' + $.json_content_box_txt + '</span></div>';
				  break;
				case 'img':
		...
		if ( typeof JSONObject.numberline_object !== 'undefined' ) {
			$.each(JSONObject.numberline_object, function(index) {
                var style = "height:80px;width:"+(JSONObject.numberline_object[index].lineLength)  +
                            "px;top:"+JSONObject.numberline_object[index].top +
                            "px;left:"+JSONObject.numberline_object[index].left +
                            "px;background-color:white"; 
                contentArr += '<div id="canvas'+index+'" class="numberline_object" data-block="numberline_object" data-id="'+index+'" style="position:absolute;'+style+'"></div>';
            });
        }
	...
	// END new content wrapper
        ...
        if (pageName[2] !== "builder.php" && media_path !== "") {
	    	$.getScript("js/ps27.js");
	    	$.getScript("Grading/grading.js");
        } else if (media_path !== "") {
	    	$.getScript("templates/ps27/js/rgbcolor.js");
	    	$.getScript("templates/ps27/js/StackBlur.js");
	    	$.getScript("templates/ps27/js/canvg.js");
	    	$.getScript("templates/ps27/js/lib/html2canvas.js");
	    	$.getScript("templates/ps27/js/ps27.js");
	    	$.getScript("templates/ps27/Grading/grading.js");
        }
        polljQuery(30);
        ...
        if ($(".cover_slide").length) {
		    $( ".cover_slide" ).draggable($.cover_slide_options);
        }
	}, 
    numberlines: [],
    load_numberlines: function() {
        var cogAPI = false;
		if(typeof HTML5RiaAPI !== "undefined") {
            var cogAPI = HTML5RiaAPI.getInstance();
            cogAPI.setQuestionStateGetFromAPPFunction(getState);
            cogAPI.setCaptureImageFromAPPFunction(getScreenShot);
        }
        var self = this;
        self.numberlines = [];
        $.each(JSONObject.numberline_object, function(nl){
            var p = new ps27();
            $("#canvas"+nl+" svg").remove();
            p.createCanvas(nl);
            self.numberlines.push(p);
            JSONObject.numberline_object[nl].font_size = JSONObject.font_size;
            p.refresh(JSONObject.numberline_object[nl]);
        });
        $("#controls").remove();
        if (pageName[1] === 'builder.php' && $(".edit_mode")[0].value === "edit") {
            ...
            $("#buttons").append("<button id='sa' class='buttons'>Show</button>");
            $("#buttons").append("<button id='sb' class='buttons'>Set Answer</button>");
            $("#buttons").append("<button id='sc' class='buttons'>Test Submit</button>");
            $("#sb")[0].onclick = set_numberlines;
            $("#sc")[0].onclick = function(){alert(check_numberlines(getState())?"Pass":"Fail");};
            $("#sa")[0].onclick = showA;
            $("#cb")[0].onclick = this.reset_numberlines;
        } else if (pageName[1] === 'builder.php' && $(".edit_mode")[0].value === "student_view") {
            $('<div id="controls"> <div id="buttons"> <button id="cb">Clear</button> </div> </div>').insertAfter("#wrapper");
            $("#buttons").append("<button id='sc' class='buttons'>Submit</button>");
            $("#sc")[0].onclick = function(){alert(check_numberlines(getState())?"Pass":"Fail");};
            $("#cb")[0].onclick = this.reset_numberlines;
    ...
    reset_numberlines: function() {
        window.nlLoaded = false;
        $(".canvg").remove();
        var self = page_builder_app;
        for (var i = 0; i < self.numberlines.length; i++) {
            self.numberlines[i]._states = [];
            self.numberlines[i]._sectionColors = [];
...
function withjQuery ($) {
    page_builder_app.load_numberlines();
}
function polljQuery (time) {
  if (window.jQuery && $(".numberline_object").length == JSONObject.numberline_object.length && window.ps27 && window.canvg && window.html2canvas && window.MathJax) {
