...
var page_builder_app = {
	...
	pageBuilder: function(event) {
		console.log("ps54 pageBuild.pageBuilder --- ");
		...
		if (typeof JSONObject.content_box1 !== 'undefined') {

			contentArr += '<div id="content_box_set" class="content_boxs" data-id="0">';

			$.each(JSONObject.content_box1, function(index) {
				// content_box Data				
				$.json_content_box_id = JSONObject.content_box1[index].id;
				$.json_content_box_type = JSONObject.content_box1[index].type;
				$.json_content_box_txt = JSONObject.content_box1[index].text;
				$.json_content_box_name = JSONObject.content_box1[index].name;
				$.json_content_box_top = JSONObject.content_box1[index].top;
				$.json_content_box_left = JSONObject.content_box1[index].left;
				$.json_content_box_width = JSONObject.content_box1[index].width;
				...
				switch ($.json_content_box_type) {
					case 'txt':
						contentArr += '<div id="content_box_' + $.json_content_box_id + '" style="' + $.position + $.add_json_width + '" class="content_box" data-block="content_box1" data-id="' + $.json_content_box_id + '" data-type="' + $.json_content_box_type + '" data-name="' + $.json_content_box_name + '"><span class="txt">' + $.json_content_box_txt + '</span></div>';
						break;
					case 'img':
						contentArr += '<div id="content_box_' + $.json_content_box_id + '" style="' + $.position + '" class="content_box" data-block="content_box1" data-id="' + $.json_content_box_id + '" data-type="' + $.json_content_box_type + '" data-name="' + $.json_content_box_name + '"><img src="' + media_path + '/images/' + $.json_content_box_name + '" style="' + $.add_json_width + '" class="content_img"><div class="print_width"></div></div>';
						break;
					case 'hide':
						contentArr += '';
						break;
		...
		if (typeof JSONObject.coordinate_object !== 'undefined') {
			...
			$.each(JSONObject.coordinate_object, function(index) {
				...
				contentArr += '<div id="controls" style="position:absolute;left:0;top:-50px;"><button id="connect" style="'+buttonStyles+' '+connect+'">Connect</button><button id="resetButton" style="'+buttonStyles+'">Reset</button></div>';
...
function withjQuery (isTemplate) {
    ps54.init(JSONObject, isTemplate);
    page_builder_app.build_keypad_events();
    loadKeyPads();
}
function polljQuery (time, isTemplate) {
  if (window.jQuery && $(".content_boxs").length > 0 && window.ps54) {
