var ps29 = {
		...
		$.each(grid, function(index) {
			...
			if(item[1] !== "") {
				...
				$.each(colors, function(index2) {
					if(ps29.cellColors[colors[index2]]) {
	...
	init : function (data) {
		...
		if(typeof(HTML5RiaAPI) !== "undefined") {
			...
			cogAPI.setQuestionStateGetFromAPPFunction(ps29.getState);
            cogAPI.setCaptureImageFromAPPFunction(ps29.getCanvas);
		...
		$("#submitButton").click(function() {
			var correct = GradeCogneroQuestion(ps29.getState(),1);
		...
		$(".shading_object_cell").on("mouseenter" ,function(event) { 
			...
			// Clear cell
			ps29.clearCell(event.currentTarget, ps29.cellColors);
			...
			var count = 0;
			$.each(ps29.cellColors, function(index) {
				
				if($.inArray(index, cellColorData) !== -1) {
					cell = $(event.currentTarget).find("." + ps29.cellColors[index].cell)[0];
        ...
        if (typeof(prevState) !== "undefined") {
            ps29.setState(prevState);
        }
        MathJax.Callback.Queue([ps29.captureScreenShot]);
	},
	
	clear : function() {
		var cells = $(".shading_object_cell");
		$.each(cells, function(index) {		
			ps29.clearCell(cells[index], ps29.cellColors);
		});
    ...
    getCanvas: function () {
        if (ps29.ssCanvas !== null) 
            return ps29.ssCanvas.toDataURL();
