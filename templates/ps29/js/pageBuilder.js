	...
	contentArr += '<div id="content" data-template="'+template+'" data-grades="'+$.json_grades+'" data-layout="'+$.json_layout+'" data-id="0" data-font="'+set_font_size+'"><div class="content">'; //data-iteration="'+iteration+'"
		...
		contentArr += '<div id="drag_object_set" class="drag_objects" data-id="0">';
			...
			$.each(JSONObject.drag_object, function(index) { 
				...
				if ( $.json_drag_object_top !== 'default' ) {
					...
					$.position = 'style="position:absolute;top:'+hOffset+'px;"';
				}
		...
		contentArr += '<div id="shading_object_set" class="shading_objects" data-id="0">';
			...
			$.each(JSONObject.shading_object, function(index) { 
				...
				contentArr += '<div id="shading_object_'+$.json_shading_object_id+'" class="shading_object '+$.json_shading_object_border_style+'" data-id="'+$.json_shading_object_id+'" data-block="shading_object" data-colors="'+$.shading_object_color_array+'" data-show-labels="'+$.json_shading_object_show_labels+'" data-multiple-cells="'+$.json_shading_object_multiple_cell+'" style="z-index:100;'+$.shading_object_dimensions+' '+$.shading_object_position+'">';
				...
				{ 	
					...
					if ( $.json_shading_object_red_check == 'true' && $.json_shading_object_red_array != '' ) {
						if ( $.inArray( $n, $.json_shading_object_red_array ) != -1 ) { $.data_option_red_mark = 'R'; } else { $.data_option_red_mark = ''; }						
					
					} else { $.data_option_red_mark = ''; }
					
					if ( $.json_shading_object_blue_check == 'true' && $.json_shading_object_blue_array != '' ) {
						if ( $.inArray( $n, $.json_shading_object_blue_array ) != -1 ) { $.data_option_blue_mark = 'B'; } else { $.data_option_blue_mark = ''; }						
					} else { $.data_option_blue_mark = ''; }
					
					if ( $.json_shading_object_green_check == 'true' && $.json_shading_object_green_array != '' ) {
						if ( $.inArray( $n, $.json_shading_object_green_array ) != -1 ) { $.data_option_green_mark = 'G'; } else { $.data_option_green_mark = ''; }						
					} else { $.data_option_green_mark = ''; }
					
					if ( $.json_shading_object_yellow_check == 'true' && $.json_shading_object_yellow_array != '' ) {
						if ( $.inArray( $n, $.json_shading_object_yellow_array ) != -1 ) { $.data_option_yellow_mark = 'Y'; } else { $.data_option_yellow_mark = ''; }						
					} else { $.data_option_yellow_mark = ''; }
					...
					contentArr += '<div id="cell_'+i+'" class="shading_object_cell" data-id="'+i+'" data-color="" style="width: '+$.cells_width+'px; height: '+$.cells_height+'px;">';
					contentArr += '<div class="red_cell" style="width: 0%; height: '+$.cells_height+'px;z-index:-100;">'+$.data_option_red_mark+'</div>';	
					contentArr += '<div class="blue_cell" style="width: 0%; height: '+$.cells_height+'px;z-index:-100;" >'+$.data_option_blue_mark+'</div>';
					contentArr += '<div class="green_cell" style="width: 0%; height: '+$.cells_height+'px;z-index:-100;" >'+$.data_option_green_mark+'</div>';
					contentArr += '<div class="yellow_cell" style="width: 0%; height: '+$.cells_height+'px;z-index:-100;" >'+$.data_option_yellow_mark+'</div>';	
					contentArr += '<span class="txt" style="width: '+$.cells_width+'px; height: '+$.cells_height+'px;z-index:110;">'+$.label_value+'</span>';
					contentArr += '</div>';
					$.label_value ++;
				}
...
function polljQuery (time) {
  if (window.jQuery && $(".shading_object_cell").length > 0 && window.ps29) {
...
function touchHandler(event) {
    ...
    switch(event.type)
    {
        ...
        case "touchmove": type="mouseenter"; lastTouchDown = -1; break;        
...
function initTouch() {
    $(".shading_object")[0].addEventListener("touchstart", touchHandler, true);
    $(".shading_object")[0].addEventListener("touchmove", touchHandler, true);
    $(".shading_object")[0].addEventListener("touchend", touchHandler, true);
    $(".shading_object")[0].addEventListener("touchcancel", touchHandler, true);
}
var JSON = JSON || {};
