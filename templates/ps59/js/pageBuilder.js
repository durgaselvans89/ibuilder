...
var page_builder_app = {
	...
	pageBuilder: function(event) {
		console.log("ps59 pageBuild.pageBuilder --- ");
		...
		if (numQuadrants > 1) {
			...
			for (var i = 0; i < posX.length; i++) {
				...
				$(posX[i]).css({
					top: top + 2,
					left: number*blocksize_x - $(posX[i]).width()/2 + 1, // 1 for border
				});
			}
...
function withjQuery (isTemplate) {
    ps59.init(JSONObject, isTemplate);
    page_builder_app.build_keypad_events();
    loadKeyPads();
}
function polljQuery (time, isTemplate) {
  if (window.jQuery && $(".content_boxs").length > 0 && window.ps59) {
